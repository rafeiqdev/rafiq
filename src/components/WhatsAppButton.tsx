import { useTranslation } from 'react-i18next';
import { track } from '../lib/analytics';
import { ContactBanner } from './ContactBanner';

/** Reads VITE_WHATSAPP_NUMBER (digits only); pass a per-page prefilled
 *  message via the `message` prop. `floating=false` (the default usage, next
 *  to EmergencyLegalFAB) renders the same ContactBanner card shape in
 *  WhatsApp green; `floating=true` is the small fixed circle, kept for any
 *  future standalone use. */
const NUMBER = String(import.meta.env.VITE_WHATSAPP_NUMBER ?? '').replace(/\D/g, '');
const CONFIGURED = !!NUMBER && NUMBER !== '905000000000';

// Warn in dev so the missing/placeholder number gets noticed during setup, but
// the button itself is hidden in EVERY environment (see the render guard below)
// so production never hands a customer off to a wrong or empty chat.
if (import.meta.env.DEV && !CONFIGURED) {
  // eslint-disable-next-line no-console
  console.warn('[Rafiq] VITE_WHATSAPP_NUMBER is missing or still the placeholder (905000000000). The WhatsApp button is hidden until it is set.');
}

function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden>
      <path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 0 1 8.413 3.488 11.824 11.824 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
    </svg>
  );
}

export function WhatsAppButton({ message, floating = false }: { message?: string; floating?: boolean }) {
  const { t } = useTranslation();
  if (!CONFIGURED) return null;
  const text = encodeURIComponent(message || t('whatsapp.default'));
  const href = `https://wa.me/${NUMBER}?text=${text}`;

  if (!floating) {
    return (
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <ContactBanner
          icon={<WhatsAppIcon className="w-4 h-4 text-white" />}
          title={t('whatsapp.title')}
          subtitle={t('whatsapp.subtitle')}
          cta={t('whatsapp.cta')}
          href={href}
          ariaLabel={t('whatsapp.aria')}
          onClick={() => track('whatsapp_clicked', { target: 'inline_button' })}
          gradient="bg-gradient-to-r from-green-600 via-green-500 to-green-600"
          border="border-green-400/40"
          ctaTextColor="text-green-700"
        />
      </div>
    );
  }

  return (
    <a
      href={href}
      target="_blank"
      rel="noreferrer"
      aria-label={t('whatsapp.aria')}
      onClick={() => track('whatsapp_clicked', { target: 'floating_button' })}
      className="fixed bottom-5 end-5 z-40 flex items-center justify-center w-14 h-14 rounded-full text-white shadow-float animate-float-pulse transition-transform hover:scale-105 active:scale-95"
      style={{ backgroundColor: '#25d366' }}
    >
      <WhatsAppIcon className="w-7 h-7" />
    </a>
  );
}
